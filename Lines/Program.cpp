...
int main(int argc, char* argv[])
{
	try
	{
		...
		gfx::SetLineSafe(
			&frameBuffer, 
			{ 0, 0 }, 
			{ static_cast<int>(frameBuffer.GetWidth() - 1), static_cast<int>(frameBuffer.GetHeight() - 1) },
			{ 0, 255, 0, 0 });

		// Линия с левого нижнего угла в правый верхний
		gfx::SetLineSafe(
			&frameBuffer, 
			{ 0, static_cast<int>(frameBuffer.GetHeight() - 1) },
			{ static_cast<int>(frameBuffer.GetWidth() - 1), 0 },
			{ 0, 255, 0, 0 });

		// Линия с справа на лево по центру высоты кадра
		gfx::SetLineSafe(
			&frameBuffer, 
			{ 0, static_cast<int>((frameBuffer.GetHeight()-1) / 2) }, 
			{ static_cast<int>(frameBuffer.GetWidth() - 1), static_cast<int>((frameBuffer.GetHeight() - 1) / 2) },
			{ 0, 255, 0 , 0 });
